  ...
  update(
    ...
    switch (this.playerBehavior) {
      case PlayerBehavior.IDLE:
        ...
        if (Phaser.Input.Keyboard.JustDown(keyF) && item?.itemType === ItemType.CHAIR) {
          ...
          this.scene.time.addEvent({
            ...
            callback: () => {
              ...
              if (chairItem.itemDirection) {
                this.setPosition(chairItem.x + sittingShiftData[chairItem.itemDirection][0], chairItem.y + sittingShiftData[chairItem.itemDirection][1]).setDepth(
                  chairItem.depth + sittingShiftData[chairItem.itemDirection][2]
                );
                // also update playerNameContainer velocity and position
                this.playContainerBody.setVelocity(0, 0);
                this.playerContainer.setPosition(chairItem.x + sittingShiftData[chairItem.itemDirection][0], chairItem.y + sittingShiftData[chairItem.itemDirection][1] - 30);
              }
        ...
        let vx = 0;
        let vy = 0;
        if (cursors.left?.isDown || cursors.A?.isDown || mouse?.isDown) vx -= speed;
        if (cursors.right?.isDown || cursors.D?.isDown || mouse?.isDown) vx += speed;
        if (cursors.up?.isDown || cursors.W?.isDown || mouse?.isDown) {
          vy -= speed;
          this.setDepth(this.y); //change player.depth if player.y changes
        }
        if (cursors.down?.isDown || cursors.S?.isDown || mouse?.isDown) {
          vy += speed;
          // this.setDepth(this.y) //change player.depth if player.y changes
        }
...
Phaser.GameObjects.GameObjectFactory.register('myPlayer', function (this: Phaser.GameObjects.GameObjectFactory, x: number, y: number, texture: string, id: string, frame?: string | number) {
